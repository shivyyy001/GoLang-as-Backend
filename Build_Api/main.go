...
func main() {
	fmt.Println("CRUD operations using go")
	r := mux.NewRouter()
	...
	courses = append(courses, Course{CourseId: "2",
		CourseName:  "ReactJs",
		CoursePrice: 299,
		Author: Author{Fullname: "shivansh",
			Website: "lco.dev"}})
...
func createOneCourse(w http.ResponseWriter, r *http.Request) {
	...
	// if same coursename exists don't create
	for _, item := range courses {
		if item.CourseName == course.CourseName {
			json.NewEncoder(w).Encode("Creation failed as course with same name exists")
			return
		}
	}
...
func updateOneCourse(w http.ResponseWriter, r *http.Request) {
	...
	for index, course := range courses {
		if course.CourseId == params["id"] {
			...
			var courseDecoded Course
			_ = json.NewDecoder(r.Body).Decode(&courseDecoded)
			courseDecoded.CourseId = params["id"]
			courses = append(courses, courseDecoded)
			json.NewEncoder(w).Encode(courseDecoded)
			return
		}
	}
